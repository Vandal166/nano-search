...
public class FilterOptionsViewModel : INotifyPropertyChanged
{
    ...
    public FilterOptionsViewModel(FileFilterOptions options)
    {
        _options = options;
        ...
        FileAttributeFlags = new ObservableCollection<EnumFlagViewModel>(
            Enum.GetValues(typeof(FileAttributes))
                .Cast<FileAttributes>()
                .Select(f => new EnumFlagViewModel(f, this))
        );
